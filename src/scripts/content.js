...
class GoogleSERPScraper {
    ...
    getHistoryStats() {
        const stats = {
            totalEntries: this.searchHistory.data.length,
        ...
        this.searchHistory.data.forEach(entry => {
            ...
            if (entry.results.found) {
                ...
                foundCount++;
                totalPositions += entry.results.position;

                // Position distribution
                const pos = entry.results.position;
                if (pos <= 3) stats.positionDistribution.top3++;
                else if (pos <= 10) stats.positionDistribution.top10++;
                else if (pos <= 30) stats.positionDistribution.top30++;
                else stats.positionDistribution.beyond30++;
            } else {
        ...
        if (foundCount > 0) {
            stats.averagePosition = Math.round(totalPositions / foundCount);
        }

        // Convert top keywords to sorted array
    ...
    async exportSearchHistory(format = 'json') {
        ...
        switch (format.toLowerCase()) {
            ...
            case 'csv':
                return this.exportHistoryAsCSV(historyData);
    ...
    exportHistoryAsCSV(historyData) {
        const headers = [
            'Date', 'Keyword', 'Found', 'Position', 'Match Type', 
            'Confidence', 'Total Results', 'URL'
        ];
    ...
    // Enhanced scraping method to integrate with history
    async scrapeWithHistory(options = {}) {
        const results = this.scrapeSearchResults(options);
        
        if (options.keyword && this.searchHistory.enabled) {
            const matchResult = this.findKeywordRanking(options.keyword, results, options);
            
            // Save to history
            await this.saveSearchResult(options.keyword, results, matchResult);
            
            // Return enhanced result with history context
            return {
                ...matchResult,
                historyContext: {
                    previousSearches: await this.getPreviousSearches(options.keyword, 5),
                    trending: this.analyzeKeywordTrend(options.keyword)
                }
            };
        }
    ...
    async getPreviousSearches(keyword, limit = 5) {
        const historyResult = await this.getSearchHistory({
            keyword: keyword,
            limit: limit
        });
        
        return historyResult.history.map(entry => ({
            date: new Date(entry.timestamp).toLocaleDateString(),
            position: entry.results.position,
            found: entry.results.found,
            confidence: entry.results.confidence
        }));
    }

    analyzeKeywordTrend(keyword) {
        const recentSearches = this.searchHistory.data
            .filter(entry => entry.keyword.toLowerCase() === keyword.toLowerCase())
        ...
        const positions = recentSearches
            .filter(entry => entry.results.found)
            .map(entry => entry.results.position);

        if (positions.length < 2) {
            return { trend: 'insufficient_found_data', message: 'Need more successful searches' };
        }
        ...
        const change = firstAvg - secondAvg; // Positive = improving (lower position numbers)

        let trend = 'stable';
        let message = 'Position relatively stable';

        if (change > 5) {
            trend = 'improving';
            message = `Position improved by ~${Math.round(change)} places`;
        } else if (change < -5) {
            trend = 'declining';
            message = `Position declined by ~${Math.round(Math.abs(change))} places`;
        }
